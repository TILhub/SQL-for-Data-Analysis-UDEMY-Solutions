    ...
    SELECT SUM(standard_amt_usd)/SUM(standard_qty) AS unit_cost
    FROM orders
    ...
    the mean amount of each paper type purchased per order. Your final answer should have 6 values 
    ...
    advanced than what we have covered so far try finding - what is the MEDIAN total_usd 
    spent on all orders?
	...
	the total sales for each companys orders in usd and the company name.
	...
	in terms of total usd. Provide only two columns - the account name and 
	the total usd. Order from smallest dollar amounts to largest.
	...
	SELECT r.name, COUNT(r.name) num_reps
	FROM region r
	JOIN sales_reps s
	ON r.id=s.region_id
	GROUP BY r.name
    ...
    SELECT accounts.id, accounts.name, COUNT(*) AS chan_bel
    FROM accounts
    JOIN web_events
	...
	SELECT DATE_PART('year',occurred_at) ord_yr,COUNT(*) sales_total
	FROM orders
	GROUP BY 1
	ORDER BY 2 DESC;
	...
	SELECT DATE_PART('month',occurred_at) ord_mnth,COUNT(*) sales_count
	FROM orders
	GROUP BY 1
	ORDER BY 2
    ...
    performing sales reps, which are sales reps associated with more than 200 orders or more than 750000 in total sales. 
