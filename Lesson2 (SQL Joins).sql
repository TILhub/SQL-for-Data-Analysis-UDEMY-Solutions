...
3)	Provide a table for all web_events associated with account name of Walmart. There should be three columns. Be sure to include the primary_poc, time of the event, and the channel for each event. Additionally, you might choose to add a fourth column to assure only Walmart events were chosen.
	SELECT accounts.primary_poc,web_events.channel
	FROM web_events
	JOIN accounts
...
4)	Provide a table that provides the region for each sales_rep along with their associated accounts. Your final table should include three columns: the region name, the sales rep name, and the account name. Sort the accounts alphabetically (A-Z) according to account name.
	SELECT region.name ,accounts.name, sales_rep.name, accounts.name
	FROM sales_rep
	JOIN region
	ON sales_rep.region_id=region.id
	JOIN accounts
...
5)	Provide the name for each region for every order, as well as the account name 
	...
	should have 3 columns: region name, account name, and unit price. A few accounts have 0 for total,
...
6)  Provide a table that provides the region for each sales_rep along with their associated accounts. 
	...
	he sales rep name, and the account name. Sort the accounts alphabetically (A-Z) according to account name.
...
8)  Provide a table that provides the region for each sales_rep along with their associated accounts. 
	...
	Your final table should include three columns: the region name, the sales rep name, and the account name. 
	ort the accounts alphabetically (A-Z) according to account name.
...
9)  Provide the name for each region for every order, as well as the account name and the unit 
...
11) Provide the name for each region for every order, as well as the account name and the unit 
...
13) Find all the orders that occurred in 2015. Your final table 
